...
__global__ void ExtractDistributedQrRKernel(
    ...
    int32_t process_col, int64_t qr_local_rows, int64_t r_local_rows,
    int64_t r_local_cols) {
  const int64_t element =
      static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  const int64_t element_count = r_local_rows * r_local_cols;
  if (element >= element_count) return;

  const int64_t local_row = element % r_local_rows;
  const int64_t local_col = element / r_local_rows;
  ...
  r[element] = global_row <= global_col
                   ? packed_qr[local_row + local_col * qr_local_rows]
                   : DataType{};
}
...
cudaError_t LaunchExtractDistributedQrR(
    ...
    int32_t process_row, int32_t process_col, int64_t qr_local_rows,
    int64_t r_local_rows, int64_t r_local_cols) {
  const int64_t element_count = r_local_rows * r_local_cols;
  ...
  ExtractDistributedQrRKernel<DataType>
      <<<block_count, kQrExtractBlockSize, 0, cuda_stream>>>(
          ...
          qr_local_rows, r_local_rows, r_local_cols);
...
cudaError_t ExtractDistributedQrR(
  ...
  if (cuda_stream == nullptr || packed_qr == nullptr || r == nullptr || n < 0 ||
      ...
      process_row < 0 || process_col < 0 || qr_local_rows < r_local_rows ||
      r_local_rows < 0 || r_local_cols < 0) {
  ...
  switch (dtype) {
    case CUDA_R_32F:
      return LaunchExtractDistributedQrR<float>(
          ...
          process_row, process_col, qr_local_rows, r_local_rows, r_local_cols);
    case CUDA_R_64F:
