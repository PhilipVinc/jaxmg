...
def run_gpu_test(
    ...
    *,
    ...
    without ``mesh``/``matrix_specs`` inside the caller's jit (``inferred``),
    or a check that an unsupported device order is rejected
    (``invalid_order``).
    """
    if interface not in ("public", "context", "inferred", "invalid_order"):
